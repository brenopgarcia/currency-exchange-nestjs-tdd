...
import { ExchangeService, CurrenciesService } from './exchange.service';
...
describe('ExchangeService', () => {
  ...
  describe('convertAmount', () => {
    it('should be throw if called invalid params', async () => {
      await expect(
        service.convertAmount({ from: '', to: '', amount: 0 }),
      ).rejects.toThrow(new BadRequestException());
    });

    it('should be not throw if called valid params', async () => {
      await expect(
